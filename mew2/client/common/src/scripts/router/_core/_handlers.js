/**
 * @file _core/handlers.js
 *
 * Prepare all route handlers; set up hooks, callbacks, validation, etc.
 * Handlers are dynamically registered by adding a new handler to the _app/handlers folder.
 *
...
define([
  'router/_handlers',
  'router/_app/_routeOrder',
  'router/_app/_hooks',
  'router/_app/_facets'
], function(handlers, routeOrder, Hooks, Facets) {
  'use strict';
  ...
  var handlerDefaults = {
    ...
    view: {
      ...
      /**
       * Used by the Nav View; set the menu ID for the current handler. Defaults to 'top'
       *
       * {this} is a reference to the view controller
       *
       * @param data {Object} the data object is passed in, so handlers can supply a dynamic menuID based on available data
       * @return menuId {String} the actual menuId string for setting active menu state
       *
       * @see viewController.route();
       */
      getMenuId: function() {
        return 'top';
      }
  ...
  var callback = function() {
    ...
    // Set up facets if they exist in the url
    if (this.currentRoute.params.facetKeys && this.currentRoute.params.facetValues) {
      var facetData = Facets.prepare.call(this, this.currentRoute.params.facetKeys, this.currentRoute.params.facetValues);
      _.extend(data, facetData || {});
    }
   ...
   * Sorts handler names against _.uniq(_.values(routeOrder)).
   * keys exist only for full visiblility of routes in a single place.
   *
   * @see _core/handlers
   * @see _app/routeOrder
   */
  var routes = _.uniq(_.values(routeOrder));

  // Sort handlers based on order supplied via `routes` variable
