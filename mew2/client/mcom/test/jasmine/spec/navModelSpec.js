define([
  ...
  describe('navModel', function() {
    ...
    describe('#createNavMenu', function() {

      /*it('should use the top level menu if the menu cannot be found', function() {
        navModelInstance.set('menus', {});
        spyOn(navModelInstance, 'setParentMenuItems');
        spyOn(navModelInstance, 'addNavItem');
        spyOn(navModelInstance, 'setChildMenuItems');
        navModelInstance.createNavMenu('doesnotexist');
        expect(navModelInstance.setParentMenuItems).toHaveBeenCalledWith('top');
      });*/
    });
